...
/**
 * Class represents basic functions with board
 * 
...
public class NFCCommand {
	...
	/**
	 * the function send an Inventory command (0x26 0x01 0x00) the argument
	 * myTag is the intent triggered with the TAG_DISCOVERED
	 */
		...
		while (errorOccured != 0) {
			try {
				...
				if (response[0] == (byte) 0x00 || response[0] == (byte) 0x01) {
					// Used for DEBUG : Log.i("NFCCOmmand", "SENDED Frame : " + Helper.ConvertHexByteToString((byte) 0x26) + " " +
					// Helper.ConvertHexByteToString((byte) 0x01) + " " + Helper.ConvertHexByteToString((byte) 0x00) );
					errorOccured = 0;
				}
			}
			catch (Exception e) {
	...
	/**
	 * the function send an Get System Info command (0x02 0x2B) the argument
	 * myTag is the intent triggered with the TAG_DISCOVERED
	 */
	public static byte[] sendGetSystemInfoCommandCustom(Tag myTag, DataDevice ma) {
		...
		while (errorOccured != 0) {
			try {
				...
				// response 01 = error sent back by tag (new Android 4.2.2) or BC )
				if (response[0] == (byte) 0x00 || response[0] == (byte) 0x01) {
					// Used for DEBUG : Log.i("NFCCOmmand", "SENDED Frame : " +
					// Helper.ConvertHexByteToString((byte) 0x26) + " " +
					// Helper.ConvertHexByteToString((byte) 0x01) + " " +
					// Helper.ConvertHexByteToString((byte) 0x00) );
					errorOccured = 0;
				}
			}
			catch (Exception e) {
		...
		if (response[9] == (byte) 0xE0) {
			...
			if (response[8] == (byte) 0x02) {
				...
				if (response[7] >= (byte) 0x04 && response[7] <= (byte) 0x07) {
					...
					// ma.setProductName("detected product"); This product oen't
					// exists
					ma.setBasedOnTwoBytesAddress(true);
					boolDeviceDetected = true;
				} else {
		...
		// INVENTORY HAS NOT DETECTED DEVICE ... TRYING WITH GET SYSTEM INFO
		// ROUTINE
		else {
			int cpt = 0;
			response = new byte[] { (byte) 0xAA };
			...
			while ((response == null || response[0] == (byte) 0xAA) && cpt <= 1) {
				try {
					...
					nfcvTag.close();
					if (response[0] == (byte) 0x00) {
						ma.setBasedOnTwoBytesAddress(true); // 1st (flag=1) = 2
															// add bytes (M24LR64 FREEDOM2)
						return response;
					}
				}
				catch (Exception e) {
			...
			while ((response == null || response[0] == (byte) 0xAA) && cpt <= 1) {
				try {
					...
					nfcvTag.close();
					if (response[0] == (byte) 0x00) {
						ma.setBasedOnTwoBytesAddress(false); // 1st (flag=1) = 2
																// add bytes (M24LR64 FREEDOM2)
						return response;
					}
				}
					...
					// Used for DEBUG :
					// Log.i("Exception","Get System Info Exception " +
					// e.getMessage());
					cpt++;
				}
			}
	 ...
	 * myTag is the intent triggered with the TAG_DISCOVERED
	 */
	public static byte[] sendGetSystemInfoCommandCustom_v3(Tag myTag, DataDevice ma) {
		int cpt = 0;
		byte[] response = new byte[] { (byte) 0xAA };
		byte[] GetSystemInfoFrame = new byte[2];
		...
		while ((response == null || response[0] == 1 || response[0] == (byte) 0xAA) && cpt <= 1) {
			try {
				...
				nfcvTag.close();
				if (response[0] == (byte) 0x00) {
					ma.setBasedOnTwoBytesAddress(false); // 1st (flag=1) = 2 add
															// bytes (M24LR64
															// FREEDOM2)
					return response;
				}
	 ...
	 * myTag is the intent triggered with the TAG_DISCOVERED
	 */
	public static byte[] sendGetSystemInfoCommandCustomOld(Tag myTag, DataDevice ma) {
		...
		for (int h = 0; h <= 1; h++) {
			try {
				...
				if (response[0] == (byte) 0x00) {
					// Used for DEBUG : Log.i("NFCCOmmand", "SENDED Frame : " +
					// Helper.ConvertHexByteArrayToString(GetSystemInfoFrame));
					if (h == 0)
						ma.setBasedOnTwoBytesAddress(true); // 1st (flag=1) = 2
															// add bytes (M24LR64 FREEDOM2)
					else
						ma.setBasedOnTwoBytesAddress(false); // 2nd (flag=0) = 1
																// add bytes (LRI M24LR04 FREEDOM1 !)
	...
	/**
	 * the function send an ReadSingle command (0x0A 0x20) || (0x02 0x20) the
	 * argument myTag is the intent triggered with the TAG_DISCOVERED example :
	 * StartAddress {0x00, 0x02} NbOfBlockToRead : {0x04} the function will
	 * return 04 blocks read from address 0002 According to the ISO-15693
	 * maximum block read is 32 for the same sector
	 */
		...
		while (errorOccured != 0) {
			try {
				...
				if (response[0] == (byte) 0x00 || response[0] == (byte) 0x01) {
					errorOccured = 0;
					// Used for DEBUG : Log.i("NFCCOmmand", "SENDED Frame : " +
					// Helper.ConvertHexByteArrayToString(ReadSingleBlockFrame));
				}
			}
			catch (Exception e) {
				errorOccured++;
				// Used for DEBUG : Log.i("NFCCOmmand",
				// "Response Read Single Block" +
				// Helper.ConvertHexByteArrayToString(response));
				if (errorOccured == 2) {
					// Used for DEBUG : Log.i("Exception","Exception " +
					// e.getMessage());
					return response;
				}
			}
		}
		// Used for DEBUG : Log.i("NFCCOmmand", "Response Read Sigle Block" +
		// Helper.ConvertHexByteArrayToString(response));
		return response;
	}
	 ...
	 * example : StartAddress {0x00, 0x02} NbOfBlockToRead : {0x04} the function
	 * will return 04 blocks read from address 0002 According to the ISO-15693
	 * maximum block read is 32 for the same sector
	 */
	public static byte[] sendReadMultipleBlockCommandCustom(Tag myTag, byte[] StartAddress, byte NbOfBlockToRead, DataDevice ma) {
		...
		for (int i = 0; i <= (NbOfBlockToRead * 4) - 4; i = i + 4) {
			...
			if (temp[0] != 0x00) checkCorrectAnswer = false;
		...
		if (checkCorrectAnswer == false) FinalResponse[0] = (byte) 0xAE;

		return FinalResponse;
	}

	public static byte[] sendReadMultipleBlockCommandCustomJpg(Tag myTag, byte[] StartAddress, byte NbOfBlockToRead, DataDevice ma) {
		long cpt = 0;
		...
		if (EndOfJpgFile == false) FinalResponse[0] = (byte) 0xAE;
		if (checkCorrectAnswer == false) FinalResponse[0] = (byte) 0xAF;

		return FinalResponse;
	}

	public static byte[] sendSeveralReadSingleBlockCommands(Tag myTag, byte[] StartAddress, byte[] bytNbBytesToRead, DataDevice ma) {
		long cpt = 0;
		boolean checkCorrectAnswer = true;
	...
	public static byte[] sendSeveralReadSingleBlockCommandsNbBlocks(Tag myTag, byte[] StartAddress, byte[] bNbOfBlockToRead, DataDevice ma) {
		long cpt = 0;
		boolean checkCorrectAnswer = true;
	...
	public static byte[] sendSeveralReadSingleBlockCommandsNbBlocksJpg(Tag myTag, byte[] StartAddress, byte[] bNbOfBlockToRead, DataDevice ma) {
		long cpt = 0;
		boolean EndOfJpgFile = false;
		boolean checkCorrectAnswer = true;
	...
	public static byte[] sendReadMultipleBlockCommandCustom2(Tag myTag, byte[] StartAddress, byte[] bNbOfBlockToRead, DataDevice ma) {
		...
		for (int i = 0; i < iNumberOfSectorToRead; i++) {
			...
			if (bAddressStartRead[0] < 0) incrementAddressStart0 = ((bAddressStartRead[0] + 256) + i / 8);

			if (bAddressStartRead[1] < 0) incrementAddressStart1 = ((bAddressStartRead[1] + 256) + i * 32) - (incrementAddressStart0 * 256);
	...
	public static byte[] sendReadMultipleBlockCommandCustom2Jpg(Tag myTag, byte[] StartAddress, byte[] bNbOfBlockToRead, DataDevice ma) {
		boolean checkCorrectAnswer = true;
		int iNbOfBlockToRead = Helper.Convert2bytesHexaFormatToInt(bNbOfBlockToRead);
	...
	/**
	 * 32 blocks max Beginning from Address 0x0000
	 */
	public static byte[] sendSeveralReadMultipleBlockCommands(Tag myTag, byte[] bytNbBytesToRead, DataDevice ma) {
		long cpt = 0;
		boolean checkCorrectAnswer = true;
		...
		if (iResteOfBytes % 4 > 0) iResteOfRows += 1;
	...
	/**
	 * the function send an ReadMultiple command (0x0A 0x23) || (0x02 0x23) the
	 * argument myTag is the intent triggered with the TAG_DISCOVERED example :
	 * StartAddress {0x00, 0x02} NbOfBlockToRead : {0x04} the function will
	 * return 04 blocks read from address 0002 According to the ISO-15693
	 * maximum block read is 32 for the same sector
	...
	public static byte[] sendReadMultipleBlockCommand(Tag myTag, byte[] StartAddress, byte NbOfBlockToRead, DataDevice ma) {
		...
		// Used for DEBUG : Log.i("NFCCOmmand", "SENDED Frame : " +
		// Helper.ConvertHexByteArrayToString(ReadMultipleBlockFrame));
		...
		while (errorOccured != 0) {
			try {
				...
				errorOccured++;
				// Used for DEBUG : Log.i("NFCCOmmand",
				// "SendReadMultipleBlockCommand errorOccured " + errorOccured);
				if (errorOccured == 3) {
					// Used for DEBUG : Log.i("Exception","Exception " +
					// e.getMessage());
					// Used for DEBUG : Log.i("NFCCOmmand",
					// "Error when try to read from address  " +
					// (byte)StartAddress[0] +" "+ (byte)StartAddress[1]);
					return response;
				}
			}
		}
		// Used for DEBUG : Log.i("NFCCOmmand", "Response Read Multiple Block" +
		// Helper.ConvertHexByteArrayToString(response));
		return response;
	}

	/**
	 * the function send an WriteSingle command (0x0A 0x21) || (0x02 0x21) the
	 * argument myTag is the intent triggered with the TAG_DISCOVERED example :
	 * StartAddress {0x00, 0x02} DataToWrite : {0x04 0x14 0xFF 0xB2} the
	 * function will write {0x04 0x14 0xFF 0xB2} at the address 0002
	 */
	public static byte[] sendWriteSingleBlockCommand(Tag myTag, byte[] StartAddress, byte[] DataToWrite, DataDevice ma) {
		...
		while (errorOccured != 0) {
			try {
				...
				if (response[0] == (byte) 0x00 || response[0] == (byte) 0x01) {
					errorOccured = 0;
					// Used for DEBUG : Log.i("*******",
					// "**SUCCESS** Write Data " + DataToWrite[0] +" "+
					// DataToWrite[1] +" "+ DataToWrite[2] +" "+ DataToWrite[3]
					// + " at address " + (byte)StartAddress[0] +" "+
					// (byte)StartAddress[1]);
				}
			}
			catch (Exception e) {
				errorOccured++;
				// Used for DEBUG : Log.i("NFCCOmmand",
				// "Write Single block command  " + errorOccured);
				if (errorOccured == 2) {
					// Used for DEBUG : Log.i("Exception","Exception " +
					// e.getMessage());
					// Used for DEBUG : Log.i("WRITE",
					// "**ERROR WRITE SINGLE** at address " +
					// Helper.ConvertHexByteArrayToString(StartAddress));
					return response;
				}
			}
		}
	...
	/**
	 * the function send an Write command (0x0A 0x21) || (0x02 0x21) the
	 * argument myTag is the intent triggered with the TAG_DISCOVERED example :
	 * StartAddress {0x00, 0x02} DataToWrite : {0x04 0x14 0xFF 0xB2} the
	 * function will write {0x04 0x14 0xFF 0xB2} at the address 0002
	 */
	public static byte[] sendWriteMultipleBlockCommand(Tag myTag, byte[] StartAddress, byte[] DataToWrite, DataDevice ma) {
		...
		for (int i = 0; i < NBByteToWrite; i = i + 4) {
			...
			if (response[0] == (byte) 0x01) return response;
			cpt = 0;
		}
		return response;
	}
	...
	public static byte[] sendWritePasswordCommand(Tag myTag, byte PasswordNumber, byte[] PasswordData, DataDevice ma) {
		...
		while (errorOccured != 0) {
			try {
				...
				nfcvTag.connect();
				response = nfcvTag.transceive(PresentPasswordFrame);
				// response 01 = error sent back by tag (new Android
				// 4.2.2) or BC
				if (response[0] == (byte) 0x00 || response[0] == (byte) 0x01) {
					errorOccured = 0;
					// Used for DEBUG : Log.i("*******",
	 ...
	 * function will write {0x04 0x14 0xFF 0xB2} at the address 0002
	 */
	public static byte[] aendWriteD0configCommand(Tag myTag, byte D0configByte, DataDevice ma) {
		byte[] response = new byte[] { (byte) 0xFF };
